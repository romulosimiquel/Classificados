...
class Usuarios extends model {
	...
	public function cadastrar($nome, $email, $senha, $telefone, $premium) {
		...
		if($sql->rowCount() == 0) {
			...
			$sql = $this->db->prepare("INSERT INTO usuarios SET nome = :nome, email = :email, senha = :senha, 
				telefone = :telefone, premium = :premium");
	...
	public function login($email, $senha) {
		...
		$sql = $this->db->prepare("SELECT id, nome, premium FROM usuarios WHERE email = :email AND senha = :senha");
		$sql->bindValue(":email",$email);
		$sql->bindValue(":senha",md5($senha));
		$sql->execute();
		...
		if($sql->rowCount() > 0) {
			...
			$_SESSION['cPremium'] = $dado['premium'];
			return true;
		} else {
			return false;
		}
